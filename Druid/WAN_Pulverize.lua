...
local function CheckAbilityValue()
    ...
    if not wan.PlayerState.Status or not wan.auraData.player.buff_BearForm
        or not wan.IsSpellUsable(wan.spellData.Pulverize.id)
    then
    ...
    if not isValidUnit or wan.auraData[targetUnitToken].debuff_Pulverize then
        wan.UpdateAbilityData(wan.spellData.Pulverize.basename)
        wan.UpdateMechanicData(wan.spellData.Pulverize.basename)
        return
    end
    ...
    -- Update ability data
    local damageValue = not isTanking and math.floor(cPulverizeDmg) or 0
    local healValue = isTanking and math.floor(cPulverizeHeal) or 0
